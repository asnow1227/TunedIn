...
def update_gamestate(room):
    ...
    if room.gamestate == Room.GameState.QUEUE:
        # if the current gamestate is the QUEUE and we are requesting a gamestate update
        # we should move to the prompt round (where prompts are submitted)
        # also, set the num_players variable on the room object to the number of players in the room currently.
